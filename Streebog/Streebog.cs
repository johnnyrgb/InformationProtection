...
namespace Streebog
{
    class Streebog
    {
        // Матрица для функции L-преобразования (MixColumns)
        private ulong[] A = {
        ...
        // Таблица подстановок для функции S-преобразования (SubBytes)
        private byte[] Pi ={
        ...
        // Таблица перестановок для функции P-преобрвазования (Transposition)
        private byte[] Tau ={
        ...
        // Константы для формирования раундовых ключей в функции KeySchedule
        private byte[][] C = {
        new byte[64]{
        ...
        // Счетчик длины сообщения. Хранит суммарную длину
        // всех обраработанных данных в битах
        private byte[] N = new byte[64];

        // Сумма всех блоков сообщения. Хранит сумму содержимого блоков сообщения
        private byte[] Sigma = new byte[64];

        // Длина выходного хеша
        public int outputHashLength = 0;

        public Streebog(int outputLenght)
        {
            if (outputLenght == 512)
            {
                // Заполнение всех переменных нулями, если длина хеша 512
                for (int i = 0; i < 64; i++)
                {
                    N[i] = 0x00;
                    Sigma[i] = 0x00;
                ...
                outputHashLength = 512;
            }
            else if (outputLenght == 256)
            {
                // Заполнение всех переменных нулями, кроме iv, если длина хеша 256
                for (int i = 0; i < 64; i++)
                {
                    N[i] = 0x00;
                    Sigma[i] = 0x00;
                ...
                outputHashLength = 256;
            }
        }

        /// <summary>
        ///     Сложение двух 512-битных чисел. Переполнение старших битов отбрасывается.
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <returns></returns>
        private byte[] AddModulo512(byte[] a, byte[] b)
        {
            ...
            // Копирование массива а в tempA с дополнением нулями слева
            Array.Copy(a, 0, tempA, 64 - a.Length, a.Length);
            // Копирование массива b в tempB с дополнением нулями слева
            Array.Copy(b, 0, tempB, 64 - b.Length, b.Length);

            // Перебор от младших байтов к старшим
            for (var i = 63; i >= 0; i--)
            {
                // Сложение двух байтов из tempA и tempB и значения переноса из предыдущей итерации
                sum = tempA[i] + tempB[i] + (sum >> 8);

                // Извлечение младших 8 бит результата суммы, отбрасывая перенос
        ...
        /// <summary>
        ///     Выполнение операции XOR над двумя 512-битными числами.
        /// </summary>
        /// <param name="a"></param>
        /// <param name="b"></param>
        /// <returns></returns>
        ...
        /// <summary>
        ///     S-преобразование над 512-битным числом. 
        ///     Выполняет замену каждого байта на соответствующий state[i] из таблицы Pi.
        /// </summary>
        ...
        private byte[] S(byte[] state)
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 64; i++)
                // Используется значение индекса элемента Pi из state
                result[i] = Pi[state[i]];
            return result;
        }

        /// <summary>
        ///     P-преобразование над 512-битным числом. Переставляет байты входного массива state
        ///     согласно фиксированной таблице перестановок Tau.
        /// </summary>
        /// <param name="state"></param>
        /// <returns></returns>
        ...
        /// <summary>
        ///     L-преобразование над 512-битным числом. Смешивает биты внутри блока
        /// </summary>
        /// <param name="state"></param>
        /// <returns></returns>
        private byte[] L(byte[] state)
        {
            ...
            // Входной массив state разделяется на 8 блоков по 8 байт. Перебор блоков по 64 бита
            for (int i = 0; i < 8; i++)
            {
                // t хранит результат преобразования текущего блока
                ulong t = 0;

                // Копирование 8 байт из state в tempArray
                byte[] tempArray = new byte[8];
                Array.Copy(state, i * 8, tempArray, 0, 8);
                tempArray = tempArray.Reverse().ToArray();

                // Преобразование массива байт в массив бит
                bool[] tempBits = new bool[64];
                new BitArray(tempArray).CopyTo(tempBits, 0);
                tempBits = tempBits.Reverse().ToArray();

                // Перебор битов в блоке
                for (int j = 0; j < 64; j++)
                {
                    // Вычисление нового значения бита c использованием матрицы A (XOR)
                    if (tempBits[j] != false)
                        t = t ^ A[j];
                }

                // Копирование блока в результат
                byte[] resultBlock = BitConverter.GetBytes(t).Reverse().ToArray();
                Array.Copy(resultBlock, 0, result, i * 8, 8);
            }
            return result;
        }
        ...
        /// <summary>
        ///     Основная функция преобразования. Выполняет 12 раундов преобразования блоком сообщения.
        ///     Использует ключ K и блок сообщения m.
        /// </summary>
        /// <param name="K"></param>
        /// <param name="m"></param>
        /// <returns></returns>
        ...
        {
            // Операция XOR между ключом K и блоком m
            byte[] state = AddXor512(K, m);

            // 12 раундов преобразования
            for (int i = 0; i < 12; i++)
            {
                // Нелинейная замена байтов с использованием таблицы подстановок Pi.
                // Каждый байт state[i] заменяется на Pi[state[i]]
                state = S(state);

                // Перестановка байтов в состоянии state согласно таблице перестановок Tau
                state = P(state);

                // Смешивание битов внутри блока с использованием матрицы A
                state = L(state);

                // Генерация раундового ключа для следующего раунда
                K = KeySchedule(K, i);

                // Операция XOR между новым раундовым ключом K и состоянием state
        ...
        /// <summary>
        ///     Функция сжатия. Обновляет текущее значение хеша
        ///     на основе нового блока сообщения.
        /// </summary>
        /// <param name="N"></param>
        /// <param name="h"></param>
        /// <param name="m"></param>
        /// <returns></returns>
        private byte[] GetHash(byte[] N, byte[] h, byte[] m)
        {
            // Генерация ключа K с помощью XOR между текущим хешем и счетчиком
            ...
            // Выполнение основной функции преобразования E
            // с использованием ключа K и блока сообщения m
            byte[] t = E(K, m);

            // Смешивание предыдущего хеша с текущим результатом преобразования
            t = AddXor512(t, h);

            // Смешивание t и блока сообщения
            byte[] newHash = AddXor512(t, m);
            return newHash;
        }

        public byte[] Run(byte[] message)
        {
            // Последний блок сообщения, дополненный до 512 бит
            byte[] paddedLastBlock = new byte[64];
            ...
            // Содержимое вектора инициализации iv используется как текущее значение хеша
            Array.Copy(iv, hash, 64);

            // Массив, заполненный нулями
            byte[] N_0 = new byte[64];


            if (outputHashLength == 512)
            {
                for (int i = 0; i < 64; i++)
                {
                    N[i] = 0x00;
            ...
            // 64-битное представление числа 512. Используется для обновления счетчика N
            byte[] N_512 = BitConverter.GetBytes(512);

            // Счетчик полных 512-битных блоков сообщения, обработанных в цикле
            int inc = 0;

            // Цикл выполнеяется, пока оставшаяся длиа сообщения не меньше 512 бит
            while (len >= 512)
            {
                inc++;

                // Массив для хранения текущего блока сообщения
                byte[] currentBlock = new byte[64];
                // Копирование блока из message в tempMes
                Array.Copy(message, message.Length - inc * 64, currentBlock, 0, 64);

                // Обновление значения хеша с помощью функции сжатия
                hash = GetHash(N, hash, currentBlock);

                // Обновление счетчика N
                N = AddModulo512(N, N_512.Reverse().ToArray());

                // Обновление суммы всех блоков сообщения
                Sigma = AddModulo512(Sigma, currentBlock);

                // Уменьшение длины сообщения на 512 бит
                len -= 512;
            }

            // Оставшийся фрагмент сообщения
            byte[] lastBlock = new byte[message.Length - inc * 64];
            Array.Copy(message, 0, lastBlock, 0, message.Length - inc * 64);

            // Дополнение сообщения до 512 бит
            if (lastBlock.Length < 64)
            {
                // Заполнение нулями, кроме 1 бита
                for (int i = 0; i < (64 - lastBlock.Length - 1); i++)
                {
                    paddedLastBlock[i] = 0;
                }

                // Установка разделительного байта, равного 1
                paddedLastBlock[64 - lastBlock.Length - 1] = 0x01;

                // Собираем сообщение воедино
                Array.Copy(lastBlock, 0, paddedLastBlock, 64 - lastBlock.Length, lastBlock.Length);
            }

            // Обновление текущего значения хеша с учетом дополненного блока сообщения
            hash = GetHash(N, hash, paddedLastBlock);

            // Вычисление длины последнего блока до добавления и преобразование в байтовое значение
            byte[] lastBlockLength = BitConverter.GetBytes(lastBlock.Length * 8);

            // Обновление счетчика N
            N = AddModulo512(N, lastBlockLength.Reverse().ToArray());

            // Обновление суммы всех блоков сообщения
            Sigma = AddModulo512(Sigma, paddedLastBlock);


            hash = GetHash(N_0, hash, N);
            hash = GetHash(N_0, hash, Sigma);
            if (outputHashLength == 512)
                return hash;
            else
            {
